...
const AnnotationApproverContext = createContext<AnnotationApproverCtx | null>(
  null
);
...
const AnnotationApproverProvider = (props: AnnotationApproverProps) => {
  ...
  const [
    tagSelectionAnnotation,
    setSelectionAnnotation,
  ] = useState<ApprovableAnnotation | null>(null);
  ...
  async function onSelectTag(t: Tag) {
    ...
    const success = await annotationAPIPost(tagSelectionAnnotation, {
      ...data,
      annotated_cls,
    });
  ...
  const selectedTag = useTags().find(
    (d) => d.tag_id == tagSelectionAnnotation?.tag_id
  );
...
function useAnnotationApproved(
  annotation: Annotation
): AnnotationApprovalStatus | null {
